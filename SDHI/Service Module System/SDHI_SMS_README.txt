...
Parts in this pack conform to the new CFG file definition scheme, and as such are only compatible with KSP versions 0.22+.
...
===Usage===
 ...
 - The pack comes prepackaged with the FusTek_Sumghai.dll plugin for handling the IACBM docking port guidance fin animations; the same plugin is also used in the FusTek Station Parts Expansion, so it doesn't matter if either version gets overwritten by the other.

 - stupid_chris's RealChute Parachute Systems plugin is required, and needs to be downloaded separately
